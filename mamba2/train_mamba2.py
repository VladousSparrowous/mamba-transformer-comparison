    ...
    def pretrain(self, dataloader):
        ...
        for epoch in range(self.config.pretrain_epochs):
            ...
            for step, batch in enumerate(progress_bar):
                ...
                # Forward pass with Mamba-2 model
    ...
    def finetune(self, train_loader, val_loader, test_loader=None):
        ...
        for epoch in range(self.config.num_epochs):
            ...
            for step, batch in enumerate(progress_bar):
                ...
                # Forward pass through Mamba-2 model to get embeddings
                # We need to extract the hidden states before the LM head
                x = self.model.backbone.embedding(inputs)
                for layer in self.model.backbone.layers:
                    x = layer['mixer'](layer['norm'](x))[0] + x
            ...
            if val_acc > best_val_acc:
                ...
                patience_counter = 0
                # Save best model
                torch.save(self.model.state_dict(), "best_model_mamba2.pt")
            else:
                patience_counter += 1
        ...
        # Load best model
        if os.path.exists("best_model_mamba2.pt"):
            self.model.load_state_dict(torch.load("best_model_mamba2.pt"))
