...
class DeBERTaTrainer:
    def __init__(self, model, config, device="cuda"):
        ...
        # Separate optimizer for DeBERTa (lower LR) and classifier (higher LR)
        self.optimizer = AdamW([
            {'params': self.model.model.deberta.parameters(), 'lr': config.learning_rate},
            {'params': self.model.model.classifier.parameters(), 'lr': config.learning_rate * 10},
    ...
    def pretrain(self, dataloader):
        ...
        for epoch in range(self.config.pretrain_epochs):
            ...
            for step, batch in enumerate(progress_bar):
                ...
                # Forward pass - MLM
                logits = self.model.model.mlm_forward(
                    input_ids, 
    ...
    def finetune(self, train_loader, val_loader, test_loader=None):
        ...
        for epoch in range(self.config.num_epochs):
            ...
            for step, batch in enumerate(progress_bar):
                ...
                logits = self.model.classify(input_ids, attention_mask=attention_mask)
            ...
            if val_acc > best_val_acc:
                ...
                patience_counter = 0
                torch.save(self.model.state_dict(), "best_deberta_model.pt")
            else:
                patience_counter += 1
        ...
        # Load best model
        if os.path.exists("best_deberta_model.pt"):
            self.model.load_state_dict(torch.load("best_deberta_model.pt"))
